...
const TagsPage: React.FC<TagsPageProps> = ({ isAuthenticated }) => {
  ...
  const handleDelete = async (tag: Tag) => {
    if (
      !window.confirm(`Sunteți sigură că doriți să ștergeți această categorie? "${tag.name}"?`)
    ) {
      return;
    }
    ...
    <div className="max-w-4xl mx-auto px-4">
      <Card>
        <CardHeader className="flex justify-between items-center">
          <h1 className="text-2xl font-bold">Tag-uri</h1>
          {isAuthenticated && (
            <Button
              color="primary"
        ...
        <CardBody>
          ...
          >
            <TableHeader>
              ...
              <TableColumn>POSTĂRI CU ACEST TAG</TableColumn>
