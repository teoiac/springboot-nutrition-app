...
const PostForm: React.FC<PostFormProps> = ({
  ...
  const validateForm = (): boolean => {
    ...
    if (!title.trim()) {
      newErrors.title = 'Titlul este necesar';
    }
    if (!editor?.getHTML() || editor?.getHTML() === '<p></p>') {
      newErrors.content = 'Continutul este necesar';
    }
    if (!categoryId) {
      newErrors.category = 'Categoria trebuie selectata';
    }
      ...
      <Card>
        <CardBody className="space-y-4">
          ...
          <div className="flex justify-end gap-2 pt-4">
            ...
            >
              {initialPost ? 'Updatează' : 'Creează'} Post
            </Button>
          </div>
        </CardBody>
      </Card>
